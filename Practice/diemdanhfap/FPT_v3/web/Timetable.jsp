...
<html>
    ...
    <body>
        ...
        <div class="row" style="margin-top: 50px"> 
            <div class="col-md-2"></div>
            <div class="col-md-8">
                <form action="TableTime" method="post">
                    From:
                    <select name="start">
                        <c:forEach items="${listTime}" var="ti1">
                        <option>${ti1.getTime()}</option>
                        </c:forEach>
                    </select>

                    To:
                    <select name="end">
                        <c:forEach items="${listTime}" var="ti2">
                        <option>${ti2.getTime()}</option>
                        </c:forEach>
                    </select>
            ...
            <div class="col-md-8"> 
                <table class="table table-hover">
                    ...
                    <c:forEach items="${Timetable}" var="time">
                        <tr>
                            <td>${time.getCourseId()}</td>
                            <td>${time.getSemester()}</td>
                            <td>${time.getClassed()}</td>
                            <td>${time.getTime()}</td> 
                            <td>${time.getSlot()}</td>
                            <td>${time.getAttendance()}</td>
                        </tr>
                    </c:forEach>
